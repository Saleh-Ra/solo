...
public class Reservation implements Serializable {
    ...
    @Transient
    private LocalDateTime endTime; // Computed if not explicitly set

    private int numberOfGuests;
    private String phoneNumber;

    // Specific table ID for this reservation (derived, not stored)
    @Transient
    private int tableId;
    ...
    public LocalDateTime getEndTime() {
        if (endTime == null && reservationTime != null) {
            return reservationTime.plusMinutes(DEFAULT_DURATION_MINUTES);
        }
        return endTime;
    }
    ...
    public int getTableId() {
        if ((tableId == 0 || tableId < 0) && tables != null && !tables.isEmpty()) {
    ...
    public boolean overlaps(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        return this.reservationTime.isBefore(endTime) && this.getEndTime().isAfter(startTime);
    }
}